        ...
        <div className={s.wrapper}>
            ...
            <div className={s.container}>
                ...
                <div className={s.description}>
                    ...
                    <div className={s.genres}>{matchGenre().map((i, index) => <span key={index}
                                                                                    className={s.matchGenre}>{i}</span>)}</div>
                </div>
            </div>
            ...
            <div className={s.recommendation}>{recommendation.map(i => {
                ...
                return <div key={i.id} className={s.movie}>
                    <span>{i.title}</span>
                    <div className={s.scale}><img
                        src={i.poster_path ? `https://image.tmdb.org/t/p/w200${i.poster_path}` : basePhoto}/></div>
                </div>
            })}</div>
        </div>
